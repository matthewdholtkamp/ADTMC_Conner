...
document.addEventListener("click", (event) => {
  // Check for dispo-icon
  const justbutton = event.target.closest(".dispo-icon");
  if (justbutton) {
  ...
  // Check for close
  const currentcloser = event.target.closest(".close");
  if (currentcloser) {
  ...
  // Check for Aa (question yes-no slider)
  const currentbtn = event.target.closest(".Aa");
  if (currentbtn) {
